...
{
    texCoord += (float2)(tex2D(DisplacementSampler, texCoord * SampleWavelength + DisplacementMotionVector) * 0.2 - 0.15);
    return tex2D(TextureSampler, texCoord) * color;
}
...
float4 PsRefractAntiRefractionArea(float4 position : TEXCOORD1, float4 color : COLOR0, float2 texCoord : TEXCOORD0) : COLOR0
{
...
float4 PsLimitedRefractionArea(float4 position : TEXCOORD1, float4 color : COLOR0, float2 texCoord : TEXCOORD0) : COLOR0
{
    // Determine distance to the anti Refract position.
    float dist = saturate(distance(position.xy, RefractionVector) / RefractionVectorRange);
    float warpedCoords = (float)((tex2D(DisplacementSampler, texCoord * SampleWavelength + DisplacementMotionVector).xy - float2(0.5f, 0.5f)) * Frequency);
...
float4 PsSecondaryShader(float4 position : TEXCOORD1, float4 color : COLOR0, float2 texCoord : TEXCOORD0) : COLOR0
{
...
float4 PsRefractionSnells(float4 position : TEXCOORD1, float4 color : COLOR0, float2 texCoord : TEXCOORD0) : COLOR0
{
