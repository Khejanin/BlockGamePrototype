...
namespace GDGame.Scenes
{
    public class MainScene : Scene
    {
        ...
        private void InitCameras3D()
        {
            ...
            camera3D = new Camera3D("Main_Cam", ActorType.Camera3D, StatusType.Update, transform3D,
                ProjectionParameters.StandardDeepSixteenTen);

            camera3D.ControllerList.Add(new RotationAroundActor("Main_Cam_Rotation_Controller",
                ControllerType.FlightCamera, KeyboardManager, 1));
        ...
        private void InitGrid()
        {
            Grid grid = new Grid(new Transform3D(new Vector3(0, 0, 0), -Vector3.UnitZ, Vector3.UnitY),
                new TileFactory(game.KeyboardManager, game.ObjectManager, game.Content, game.ModelEffect));
